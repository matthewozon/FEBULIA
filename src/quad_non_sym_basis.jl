...
function basis_quad_non_sym_l(x::Cdouble,x0::Cdouble,x1::Cdouble) # a piecewise quadratic function on the range [x0,x1], phi(x0) = 0; phi(x1) = 1
    val = 0.0
    if ((x>=x0) & (x<=x1))
        # val = ((x-x0)/(x1-x0))^2
...
function basis_quad_non_sym_l(x::Array{Cdouble,1},x0::Cdouble,x1::Cdouble)
    idx1 = findall((x.>=x0) .& (x.<=x1))
    val = zeros(Cdouble,length(x))
    val[idx] = -(x[idx].-x0).*(x[idx].+x0.-2.0x1)./((x1-x0)^2)
    val
end
function basis_quad_non_sym_u(x::Cdouble,x0::Cdouble,x1::Cdouble) # a piecewise quadratic function on the range [x0,x1], phi(x0) = 1; phi(x1) = 0
    val = 0.0
    if ((x>=x0) & (x<=x1))
        val = ((x-x1)/(x0-x1))^2
    else
...
function basis_quad_non_sym_u(x::Array{Cdouble,1},x0::Cdouble,x1::Cdouble)
...
function basis_quad_non_sym(x::Cdouble,x0::Cdouble,x1::Cdouble,xm::Cdouble) # a piecewise quadratic function on the range [x0,x1], x0<xm<x1, phi(x0)=phi(x1)=0; phi(xm)=1
    val = 0.0
    if ((x>=x0) & (x<xm))
...
function basis_quad_non_sym(x::Array{Cdouble,1},x0::Cdouble,x1::Cdouble,xm::Cdouble)
...
function Basis_quad_non_sym(X::Array{Cdouble,1}) # X is a subdivision of the range [x_{min},x_{max}]
    ...
    for n in 1:N
        Fbasis[n] = (x::Cdouble->basis_quad_non_sym(x,X[n],X[n+2],X[n+1]))
    end
    Fbasis
end
...
function DBC_homogeneous_basis_quad_non_sym(X::Array{Cdouble,1})
    Fbasis = Basis_quad_non_sym(X)
...
function DBC_non_homogeneous_lower_bound_basis_quad_non_sym(X::Array{Cdouble,1})
    Fbasis = [(x::Cdouble->basis_quad_non_sym_u(x,X[1],X[2])); Basis_quad_non_syma(X)]
    # build an array of intervals
...
function DBC_non_homogeneous_upper_bound_basis_quad_non_sym(X::Array{Cdouble,1})
    Fbasis = [Basis_quad_non_sym(X); (x::Cdouble->basis_quad_non_sym_l(x,X[end-1],X[end]))]
    # build an array of intervals
...
function DBC_non_homogeneous_bounds_basis_quad_non_sym(X::Array{Cdouble,1})
    Fbasis = [(x::Cdouble->basis_quad_non_sym_u(x,X[1],X[2])); Basis_quad_non_sym(X); (x::Cdouble->basis_quad_non_sym_l(x,X[end-1],X[end]))]
    # build an array of intervals
...
function basis_quad_non_sym_BC(X::Array{Cdouble,1};BCl::String="Dirichlet",BCu::String="Dirichlet",Hl::Bool=true,Hu::Bool=true)
...
function basis_quad_non_sym_BC(X::Array{Cdouble,1},BC::BoundCond1D)
    # X is the discretized 1D space
    # BCu is the type of boundary condition applied at X[end], it can take the values Dirichlet, Neumann or Robin
    # BCl is the boundary condition at X[1]
    # Hl used if the BCl is of type Dirichlet, it indicates if it is a homogeneous BC (true) or not (false)

...
function basis_exp_l(x::Cdouble,x0::Cdouble,xm::Cdouble,tau::Cdouble) # a piecewise exp function on the range [x0,x1], x0<x2<x1
    val = 0.0
    if ((x>=x0) & (x<=xm))
        val = (1.0-exp(-tau*((x-x0)/(xm-x0))))/(1.0-exp(-tau))
    else
        val = 0.0
    end
    val
...
function basis_exp_u(x::Cdouble,xm::Cdouble,x1::Cdouble,tau::Cdouble) # a piecewise exp function on the range [x0,x1], x0<x2<x1
    val = 0.0
    if ((x>=xm) & (x<=x1))
        val = (exp(-tau*((x-xm)/(x1-xm))) - exp(-tau))/(1.0-exp(-tau))
    else
        val = 0.0
    end
    val
...
function Basis_exp(X::Array{Cdouble,1},tau::Cdouble) # X is a subdivision of the range [x_{min},x_{max}]
    ...
    for n in 1:N
        # Fbasis[n] = function expb(x::Cdouble) basis_exp(x,X[n],X[n+2],X[n+1]) end
        Fbasis[n] = (x::Cdouble->basis_exp(x,X[n],X[n+2],X[n+1],tau))
    end
    Fbasis
end
...
function DBC_homogeneous_basis_exp(X::Array{Cdouble,1},tau::Cdouble)
    Fbasis = Basis_exp(X,tau)
...
function DBC_non_homogeneous_lower_bound_basis_exp(X::Array{Cdouble,1},tau::Cdouble)
    Fbasis = [(x::Cdouble->basis_exp_u(x,X[1],X[2],tau)); Basis_exp(X,tau)]
    # build an array of intervals
...
function DBC_non_homogeneous_upper_bound_basis_exp(X::Array{Cdouble,1},tau::Cdouble)
    Fbasis = [Basis_exp(Xtau); (x::Cdouble->basis_exp_l(x,X[end-1],X[end],tau))]
    # build an array of intervals
...
function DBC_non_homogeneous_bounds_basis_exp(X::Array{Cdouble,1},tau::Cdouble)
    Fbasis = [(x::Cdouble->basis_exp_u(x,X[1],X[2],tau)); Basis_exp(X,tau); (x::Cdouble->basis_exp_l(x,X[end-1],X[end],tau))]
    # build an array of intervals
...
function basis_exp_BC(X::Array{Cdouble,1};BCl::String="Dirichlet",BCu::String="Dirichlet",Hl::Bool=true,Hu::Bool=true,tau::Cdouble=1.0)
...
function basis_exp_BC(X::Array{Cdouble,1},BC::BoundCond1D;tau::Cdouble=1.0)
    # X is the discretized 1D space
    # BCu is the type of boundary condition applied at X[end], it can take the values Dirichlet, Neumann or Robin
    # BCl is the boundary condition at X[1]
    # Hl used if the BCl is of type Dirichlet, it indicates if it is a homogeneous BC (true) or not (false)
